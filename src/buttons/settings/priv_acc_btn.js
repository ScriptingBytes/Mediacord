...
module.exports = {
    ...
    async execute (interaction, client) {
        ...
        await schema.updateOne({ userId: user.id }, { 'accountInfo.privateAcc': enums[`${privAcc}`].opposite.val })

        const privAccBtn = new ButtonBuilder()
        .setCustomId('priv_acc_btn')
        .setLabel(`Private Account (${enums[`${privAcc}`].opposite.label})`)
        .setStyle(enums[`${privAcc}`].opposite.color)
